// Node glyphs. Shape and fill are the secondary encoding that lets nine
// categories share three validated hues: no two categories have the same
// shape, fill and hue, so identity never rests on colour alone.
//
// Classic script, not an ES module: modules are blocked under file://, and this
// page has to work when someone just double-clicks index.html.

window.MAP = window.MAP || {};
(function (MAP) {
'use strict';
const FAMILY_VAR = {
  principals: '--hue-principals',
  counterparties: '--hue-counterparties',
  public: '--hue-public',
  places: '--hue-places',
};

/** Reads the live CSS custom properties so a theme change repaints correctly. */
function readPalette() {
  const cs = getComputedStyle(document.documentElement);
  const v = (name) => cs.getPropertyValue(name).trim();
  return {
    family: Object.fromEntries(Object.entries(FAMILY_VAR).map(([k, n]) => [k, v(n)])),
    surface: v('--surface'),
    plane: v('--plane'),
    ink: v('--ink'),
    ink2: v('--ink-2'),
    muted: v('--ink-muted'),
    hairline: v('--hairline'),
    edge: v('--edge'),
    edgeStrong: v('--edge-strong'),
    accent: v('--accent'),
    ageOld: v('--age-old'),
    ageNew: v('--age-new'),
    // The freshness halo. Its peak opacity is a token too: the same bloom that
    // reads as light on the dark plane reads as a heavy smudge on the light
    // one, and the difference belongs in the stylesheet with every other
    // colour decision rather than as a magic number in the renderer.
    fresh: v('--fresh'),
    freshVeil: parseFloat(v('--fresh-veil')) || 0,
    // The two logo plates. These are the only colours here that do not flip
    // with the theme, because the artwork they sit behind does not — see
    // --logo-light in the stylesheet.
    logoLight: v('--logo-light'),
    logoDark: v('--logo-dark'),
  };
}

const hex = (c) => {
  const m = /^#?([0-9a-f]{6})$/i.exec(c.trim());
  if (!m) return [128, 128, 128];
  const n = parseInt(m[1], 16);
  return [(n >> 16) & 255, (n >> 8) & 255, n & 255];
};

/** Straight sRGB lerp. Both ends of the age ramp are achromatic, so there is
 *  no hue to shift through and a simple mix reads as one continuous scale. */
function mix(a, b, t) {
  const [r1, g1, b1] = hex(a);
  const [r2, g2, b2] = hex(b);
  const k = Math.max(0, Math.min(1, t));
  return `rgb(${Math.round(r1 + (r2 - r1) * k)},${Math.round(g1 + (g2 - g1) * k)},${
    Math.round(b1 + (b2 - b1) * k)})`;
}

function withAlpha(color, alpha) {
  const [r, g, b] = hex(color);
  return `rgba(${r},${g},${b},${alpha})`;
}

/** Traces a glyph path of the given shape, centered on (x, y). */
function tracePath(ctx, shape, x, y, r) {
  ctx.beginPath();
  if (shape === 'circle') {
    ctx.arc(x, y, r, 0, Math.PI * 2);
  } else if (shape === 'square') {
    const s = r * 0.92;
    const k = Math.min(3.2, s * 0.34);
    ctx.moveTo(x - s + k, y - s);
    ctx.arcTo(x + s, y - s, x + s, y + s, k);
    ctx.arcTo(x + s, y + s, x - s, y + s, k);
    ctx.arcTo(x - s, y + s, x - s, y - s, k);
    ctx.arcTo(x - s, y - s, x + s, y - s, k);
    ctx.closePath();
  } else if (shape === 'diamond') {
    const s = r * 1.24;
    ctx.moveTo(x, y - s);
    ctx.lineTo(x + s, y);
    ctx.lineTo(x, y + s);
    ctx.lineTo(x - s, y);
    ctx.closePath();
  } else if (shape === 'hexagon') {
    const s = r * 1.1;
    for (let i = 0; i < 6; i++) {
      const a = (Math.PI / 3) * i - Math.PI / 2;
      const px = x + Math.cos(a) * s;
      const py = y + Math.sin(a) * s;
      i ? ctx.lineTo(px, py) : ctx.moveTo(px, py);
    }
    ctx.closePath();
  } else {
    ctx.arc(x, y, r, 0, Math.PI * 2);
  }
}

/** A rounded rectangle, traced. ctx.roundRect would do, but it is recent
 *  enough to be worth not depending on, and arcTo is what tracePath already
 *  builds the square glyph out of. */
function traceRoundRect(ctx, x, y, w, h, radius) {
  const k = Math.max(0, Math.min(radius, w / 2, h / 2));
  ctx.beginPath();
  ctx.moveTo(x + k, y);
  ctx.arcTo(x + w, y, x + w, y + h, k);
  ctx.arcTo(x + w, y + h, x, y + h, k);
  ctx.arcTo(x, y + h, x, y, k);
  ctx.arcTo(x, y, x + w, y, k);
  ctx.closePath();
}

/** The largest centred, axis-aligned rectangle of aspect ratio `a` (width over
 *  height) that fits inside the glyph `shape` traced at radius `r`. Returns its
 *  full width and height.
 *
 *  This is what lets a logo be fitted rather than cropped. A portrait can be
 *  cropped to fill a disc because a face survives losing its corners; a
 *  wordmark does not, so it has to be sized to the room the glyph actually has
 *  — and a diamond at the same radius has far less of it than a square. The
 *  constants below are the ones tracePath uses, and have to move with them.
 */
function fitInShape(shape, r, a) {
  const ratio = a > 0 && isFinite(a) ? a : 1;
  let w;
  if (shape === 'square') {
    const s = r * 0.92;                       // half-side
    w = Math.min(s, s * ratio);
  } else if (shape === 'diamond') {
    const s = r * 1.24;                       // half-diagonal, both axes
    w = (s * ratio) / (ratio + 1);            // |x|/s + |y|/s <= 1
  } else if (shape === 'hexagon') {
    const s = r * 1.1;                        // circumradius, pointy-top
    const R3 = Math.sqrt(3);
    w = Math.min((s * R3) / 2, (s * ratio * R3) / (R3 + ratio));
  } else {
    w = (r * ratio) / Math.sqrt(ratio * ratio + 1);  // circle
  }
  return { w: w * 2, h: (w / ratio) * 2 };
}

/** How far the glyph `shape`, traced at radius `r`, reaches from its centre
 *  along either axis, as a multiple of `r`. A diamond's points stand well
 *  outside its radius and a rounded square's sides sit inside it, so a picture
 *  cropped to fill a glyph has to be sized to the shape and not to the radius:
 *  fitted to `r` it would leave a diamond's points bare.
 *
 *  These are the same constants tracePath draws with, and have to move with
 *  them — as do fitInShape's.
 */
function shapeReach(shape) {
  if (shape === 'square') return 0.92;    // half-side
  if (shape === 'diamond') return 1.24;   // half-diagonal
  if (shape === 'hexagon') return 1.1;    // circumradius, reached at the points
  return 1;                               // circle
}

/** The blank avatar: a head and a pair of shoulders, inside a circle of radius
 *  `r` centred on (x, y). It is what a person's glyph wears when the file
 *  carries no picture of them, and it is deliberately the emptiest mark on the
 *  map — an absent portrait is a gap in what we have, not a fact about the
 *  person, so it says nothing beyond "this node is somebody".
 *
 *  Traced, not filled: the caller owns the colour, and the shoulders are meant
 *  to be cut off by whatever the caller has clipped to, the way they are in
 *  every avatar anyone has ever seen. */
function traceAvatar(ctx, x, y, r) {
  ctx.beginPath();
  ctx.arc(x, y - r * 0.26, r * 0.28, 0, Math.PI * 2);
  // A fresh subpath, or the two circles are joined by the line between them.
  ctx.moveTo(x + r * 0.66, y + r * 0.88);
  ctx.arc(x, y + r * 0.88, r * 0.66, 0, Math.PI * 2);
}

/** The gavel: a mallet at rest on its sounding block, inside a glyph of
 *  radius `r` centred on (x, y). It marks a litigation node the way the
 *  blank avatar marks a person — traced, not filled, so the caller owns the
 *  colour and can drop it straight onto the category hue.
 *
 *  Kept to three plain rectangles rather than a literal mallet cylinder: at
 *  the sizes a glyph is actually drawn at, roundness reads as noise and a
 *  head, a handle and a block are already enough to say "gavel" and nothing
 *  else on this map's legend. The head and handle share one rotation so the
 *  mallet reads as a single rigid object; the block stays level, the way a
 *  strike surface has to. */
function traceGavel(ctx, x, y, r) {
  const a = 0.72; // radians: head up-left, handle down-right
  const ax = Math.cos(a), ay = Math.sin(a);   // along the mallet's long axis
  const px = -ay, py = ax;                    // across it
  const pt = (along, across) => [
    x + along * r * ax + across * r * px,
    y + along * r * ay + across * r * py,
  ];
  const rect = (a0, a1, p0, p1) => {
    const c = [pt(a0, p0), pt(a1, p0), pt(a1, p1), pt(a0, p1)];
    ctx.moveTo(c[0][0], c[0][1]);
    for (let i = 1; i < c.length; i++) ctx.lineTo(c[i][0], c[i][1]);
    ctx.closePath();
  };
  ctx.beginPath();
  rect(-0.70, -0.30, -0.26, 0.26);   // head
  rect(-0.30, 0.55, -0.10, 0.10);    // handle
  // The block: level, not rotated with the mallet — it is what the mallet
  // strikes, not part of it. Proportions on all three rectangles are chosen
  // to keep every corner inside the diamond glyph's own outline.
  ctx.moveTo(x - 0.58 * r, y + 0.40 * r);
  ctx.lineTo(x + 0.14 * r, y + 0.40 * r);
  ctx.lineTo(x + 0.14 * r, y + 0.60 * r);
  ctx.lineTo(x - 0.58 * r, y + 0.60 * r);
  ctx.closePath();
}

/** The same glyph as an inline SVG, so the legend and the canvas cannot drift.
 *  `color` is a CSS custom-property name: the swatch paints with currentColor so
 *  a theme change repaints it without any re-render. */
function swatchSVG(shape, fill, cssVar, size = 17) {
  const color = 'currentColor';
  const style = ` style="color:var(${cssVar})"`;
  const c = size / 2;
  const r = size * 0.36;
  const hollow = fill === 'hollow';
  const attrs = hollow
    ? `fill="none" stroke="${color}" stroke-width="2.4"`
    : `fill="${color}"`;
  let inner;
  if (shape === 'circle') {
    inner = `<circle cx="${c}" cy="${c}" r="${hollow ? r - 1.2 : r}" ${attrs}/>`;
  } else if (shape === 'square') {
    const s = r * 0.92;
    const d = hollow ? s - 1.2 : s;
    inner = `<rect x="${c - d}" y="${c - d}" width="${d * 2}" height="${d * 2}" rx="${Math.min(
      3, d * 0.34)}" ${attrs}/>`;
  } else if (shape === 'diamond') {
    const s = (hollow ? r - 1.2 : r) * 1.24;
    inner = `<polygon points="${c},${c - s} ${c + s},${c} ${c},${c + s} ${c - s},${c}" ${attrs}/>`;
  } else {
    const s = (hollow ? r - 1.2 : r) * 1.1;
    const pts = Array.from({ length: 6 }, (_, i) => {
      const a = (Math.PI / 3) * i - Math.PI / 2;
      return `${(c + Math.cos(a) * s).toFixed(2)},${(c + Math.sin(a) * s).toFixed(2)}`;
    }).join(' ');
    inner = `<polygon points="${pts}" ${attrs}/>`;
  }
  // Intrinsic width/height so the glyph is correctly sized everywhere it is
  // used, not only where a container rule happens to size it.
  return `<svg class="swatch"${style} width="${size}" height="${size}"
    viewBox="0 0 ${size} ${size}" aria-hidden="true">${inner}</svg>`;
}

/** The blank avatar as an inline SVG: the same head and shoulders the canvas
 *  traces, at panel size, so the two cannot drift. The shoulders run past the
 *  foot of the box exactly as they run past the foot of the glyph. */
function avatarSVG(cssVar, size = 56) {
  const c = size / 2;
  const at = (k) => (c + c * k).toFixed(2);
  return `<svg class="avatar" style="color:var(${cssVar})" width="${size}" height="${size}"
    viewBox="0 0 ${size} ${size}" aria-hidden="true">
    <circle cx="${c}" cy="${c}" r="${c}" fill="currentColor"/>
    <circle cx="${c}" cy="${at(-0.26)}" r="${(c * 0.28).toFixed(2)}" fill="var(--surface)"/>
    <circle cx="${c}" cy="${at(0.88)}" r="${(c * 0.66).toFixed(2)}" fill="var(--surface)"/>
  </svg>`;
}

/* ------------------------------------------------------------- recency ---
   What counts as "new" on a map that is meant to answer "what is happening".
   Measured against the reader's own clock rather than the build date, so the
   window is the one the prose promises — the last thirty days — and a quiet
   month honestly shows nothing new instead of a stale banner over the last
   thing anyone happened to add.

   One constant sets the width of it, and every phrase the page says out loud
   is derived from that constant rather than typed out beside it. Widening the
   window is therefore one edit, and cannot leave the sidebar saying "week"
   over a month's worth of haloed nodes. */
const RECENT_DAYS = 30;
const RECENT_PHRASE = `the last ${RECENT_DAYS} days`;
const DAY_MS = 86400000;

/** How new a dated thing is, on 0-1 across that window: 1 on the day it is
 *  dated, falling to 0 at the far edge of it. Anything older, undated, or
 *  dated only to the month or the year returns 0, which is the same statement
 *  as "not part of this group".
 *
 *  The precision test is the point of the function, not a detail of it. "2026"
 *  is not evidence that something happened lately, and a node the record dates
 *  to a month is not one the record dates to a Tuesday — putting a freshness
 *  halo on either would claim a precision the file does not have. It would
 *  also seat it wrongly on the ramp, since a month-precision date parses to
 *  the first of the month whatever day the thing actually happened on.
 *
 *  A date ahead of the clock is treated as the newest there is rather than as
 *  an error: node dates are UTC midnight, and a reader far enough east is
 *  simply reading today's entry before UTC agrees it is today. */
function recencyOf(date, now = Date.now()) {
  if (!date || date.precision !== 'day') return 0;
  const days = (now - date.t) / DAY_MS;
  if (days <= 0) return 1;
  if (days >= RECENT_DAYS) return 0;
  return 1 - days / RECENT_DAYS;
}

/** Tier is carried by line style, so verification status survives greyscale. */
const TIER = {
  1: { label: 'Primary document', dash: [], width: 1.5, alpha: 0.92 },
  2: { label: 'Attributed or externally verified', dash: [], width: 1.1, alpha: 0.6 },
  3: { label: 'Open or unresolved', dash: [4, 4], width: 1.2, alpha: 0.55 },
};

function tierLineSVG(tier, cssVar) {
  const t = TIER[tier];
  const dash = t.dash.length ? `stroke-dasharray="${t.dash.join(' ')}"` : '';
  return `<svg class="line" style="color:var(${cssVar})" width="24" height="12"
    viewBox="0 0 24 12" aria-hidden="true">
    <line x1="1" y1="6" x2="23" y2="6" stroke="currentColor" stroke-width="${t.width + 0.6}"
      stroke-opacity="${t.alpha}" ${dash}/></svg>`;
}

MAP.shapes = {
  FAMILY_VAR, readPalette, mix, withAlpha, tracePath, traceRoundRect, fitInShape,
  shapeReach, traceAvatar, traceGavel, swatchSVG, avatarSVG, TIER, tierLineSVG,
  RECENT_DAYS, RECENT_PHRASE, recencyOf,
};
}(window.MAP));
