...
// up. What the layout is held to is legibility: every node reserves the room
// its name needs, the web is laid out in the stage's own units, and the
// opening frame is one where all 85 names can be read without touching
// anything.
// Time is layered on top two ways, neither of which imposes a linear axis —
// an age ramp on every node ring, and a scrubber that fades what had not yet
// happened.

// Classic script: this file publishes GraphView onto MAP.

window.MAP = window.MAP || {};
(function (MAP) {
'use strict';

const { readPalette, tracePath, traceRoundRect, fitInShape, shapeReach, traceAvatar,
  traceGavel, mix, withAlpha, recencyOf, TIER } = MAP.shapes;
...
// A glyph carries a picture of what it stands for — a person's face, an
// organisation's wordmark. Below this many screen pixels of radius the glyph is
// too small for either to be anything but noise, so it is left plain.
//
// One threshold for both, deliberately. A node's radius comes from its degree
// (7 + √degree × 2.5), and most of the companies in this file appear in a
// single connection, so they are the smallest glyphs on the map — a separate,
// higher bar for logos meant that at the opening zoom a handful of well-
// connected companies wore their mark and their one-connection neighbours did
// not, which reads as a rendering fault rather than as a rule. It is not one:
// having a logo is not a fact about a company. So faces and marks appear and
// disappear together, and what a reader can tell apart at this size is the same
// question for both.
const ART_MIN_PX = 7;
const ZOOM_MIN = 0.22;
const ZOOM_MAX = 4.5;
...
class GraphView {
  ...
  /** Portraits, logos and cited photographs, loaded in the background. A
   *  person wears the blank avatar and everything else wears a plain glyph
   *  until the picture arrives and repaints the map, so no frame waits on a
   *  file — and one file is loaded once however many nodes share it, which
   *  several of these do.
   *
   *  A picture that fails to load is not reported: it leaves the node exactly
   *  as one the file has no picture of, which is what it now is. */
  _loadArt() {
    this.art = new Map();
    const load = (src) => {
      if (!src || this.art.has(src)) return;
      const rec = { img: new Image(), ready: false };
      rec.img.onload = () => { rec.ready = true; this.draw(); };
      rec.img.onerror = () => { this.art.delete(src); };
      rec.img.src = src;
      this.art.set(src, rec);
    };
    for (const n of this.nodes) { load(n.image); load(n.logo); load(n.glyph); }
  }
  ...
  /** Runs separation to convergence, which takes a few dozen iterations, so
   *  the frame people actually read is always the settled one. */
  _settle() {
    [this.worldW, this.worldH] = this.worldBase;
  ...
  resize() {
    ...
    if (rect.width) {
      ...
      // dragged a node, that is their layout: it is left alone.
      if (!this._touched &&
          (before[0] !== this.stageBase[0] || before[1] !== this.stageBase[1])) {
        this._seedLayout();
        this._settle();
        this.fit(16, false);
      }
  ...
  _drawNode(n) {
    ...
    if (hollow) {
      ...
      ctx.lineWidth = 1.6;
      ctx.stroke();
      if (n.category === 'person') this._drawFace(n, r);
      else if (n.category === 'litigation') this._drawGavel(n, r);
    }

    // A logo sits inside either kind of glyph: it brings its own plate, so it
    // does not care whether the fill under it was the hue or the surface.
    if (n.logo) this._drawMark(n, r, cat?.shape || 'circle');

    // A cited photograph sits inside either kind too, and covers whichever
    // fill was under it.
    if (n.glyph) this._drawPhoto(n, r, cat?.shape || 'circle');
  ...
  /** What goes inside a person's disc: their portrait where the file carries
   *  one, and the blank avatar where it does not. Every person gets one or the
   *  other, and they are the same glyph either way — a person nobody has a
   *  picture of is not drawn as less established than a person who is
   *  photographed, because that is not what a portrait is evidence of.
   *
   *  The hue is not spent on this. It stays as a ring around whatever is
   *  inside, so a portrait never costs the node the category colour it is read
   *  by, and the ring is a share of the radius rather than a fixed width, so it
   *  does not thin to nothing on the hubs.
   *
   *  The portrait is drawn to cover the disc and cropped by it, the way every
   *  avatar is: a face letterboxed inside a circle would read as a picture of a
   *  picture. */
  _drawFace(n, r) {
    if (r * this.camera.k < ART_MIN_PX) return;
    const { ctx, palette: p } = this;
    const inner = r - Math.max(1.5, r * 0.15);
    const rec = n.image ? this.art.get(n.image) : null;
    ...
    if (rec && rec.ready) {
      ...
      const k = Math.max((inner * 2) / iw, (inner * 2) / ih);
      ctx.drawImage(rec.img, n.x - (iw * k) / 2, n.y - (ih * k) / 2, iw * k, ih * k);
    } else {
      ctx.fillStyle = p.surface;
      traceAvatar(ctx, n.x, n.y, inner);
      ctx.fill();
    }
    ctx.restore();
  }

  /** The gavel drawn inside a litigation node's glyph: the three live suits
   *  are already the one solid diamond on the map, but a diamond is also
   *  what an undecided site wears hollow, and the shape-plus-fill rule that
   *  tells them apart asks more of a reader scanning a dense web than a mark
   *  drawn on the thing itself does.
   *
   *  Traced in the surface colour and dropped straight onto the category
   *  hue, the way the blank avatar is — the hue is not spent on this either.
   *  It shares the avatar's visibility floor: below ART_MIN_PX a mallet
   *  reads as a smudge sooner than a face does, and disappearing with every
   *  other picture on the canvas is the same rule applied consistently
   *  rather than a second, stricter one for this glyph alone. */
  _drawGavel(n, r) {
    if (r * this.camera.k < ART_MIN_PX) return;
    const { ctx, palette: p } = this;
    const inner = r - Math.max(1.5, r * 0.15);
    ctx.fillStyle = p.surface;
    traceGavel(ctx, n.x, n.y, inner);
    ctx.fill();
  }

  /** What goes inside an organisation's glyph: its logo, where the file has
   *  one. Where it does not, nothing — the glyph is drawn exactly as it was
   *  before there were any logos here. There is deliberately no blank logo to
   *  match the blank avatar: a missing portrait is a gap in what we have and
   *  has to be shown as one, whereas a wordmark is only a convenience for
   *  recognising a company, and not having one says nothing about it at all.
   *
   *  A logo is fitted, not cropped. A face survives losing its corners to a
   *  disc; half a wordmark identifies nobody, so the mark is sized to the
   *  largest rectangle of its own proportions that the glyph will hold — which
   *  is why the fit is by shape and not by radius alone.
   *
   *  The plate behind it is the one colour on this canvas that ignores the
   *  theme. Several of these marks are white artwork lifted from a dark site
   *  header and several are dark artwork from a light one; the file says which
   *  (`logo_on`), and the mark keeps the background it was drawn for in both
   *  modes. As with a portrait, the hue is not spent on this: it stays as a
   *  ring around the plate, so a logo never costs the node the colour it is
   *  read by. */
  _drawMark(n, r, shape) {
    if (r * this.camera.k < ART_MIN_PX) return;
    const rec = this.art.get(n.logo);
    if (!rec || !rec.ready) return;
    const iw = rec.img.naturalWidth || 0;
    const ih = rec.img.naturalHeight || 0;
    if (!iw || !ih) return;

    const { ctx, palette: p } = this;
    const inner = r - Math.max(1.5, r * 0.15);
    const box = fitInShape(shape, inner, iw / ih);

    ctx.save();
    // The plate is the size of the mark, not the size of the glyph. A wordmark
    // is wide and shallow, and a plate blown up to fill the glyph behind one
    // would leave the node almost entirely white — which is exactly what a
    // portrait is careful not to do. Clipped to the glyph all the same, so a
    // band that reaches the sides of a square cannot poke out of a diamond.
    tracePath(ctx, shape, n.x, n.y, inner);
    ctx.clip();
    traceRoundRect(ctx, n.x - box.w / 2, n.y - box.h / 2, box.w, box.h,
      Math.min(3.2, Math.min(box.w, box.h) * 0.22));
    ctx.fillStyle = n.logoOn === 'dark' ? p.logoDark : p.logoLight;
    ctx.fill();
    // A little air between the wordmark and the plate edge, so the mark reads
    // as sitting on the plate rather than bursting out of it.
    const w = box.w * 0.86;
    const h = box.h * 0.86;
    ctx.drawImage(rec.img, n.x - w / 2, n.y - h / 2, w, h);
    ctx.restore();
  }

  /** What goes inside a glyph the file has a photograph of the thing itself:
   *  the photograph, named by whichever of the node's own citations carries
   *  `glyph: true`. It is the only picture on this canvas that is evidence —
   *  a portrait and a wordmark are conveniences and neither is cited — so
   *  there is no stand-in for its absence, as there is none for a logo's: a
   *  site nobody has photographed keeps the glyph it always had.
   *
   *  Cropped like a portrait rather than fitted like a wordmark. An aerial is
   *  ground, and ground survives losing its corners the way a face does, where
   *  a wordmark does not. It is cropped to the glyph's own outline and not to a
   *  circle, so the shape still says what the node is: the sites are diamonds,
   *  and a diamond of photograph is still legibly a diamond.
   *
   *  As with a portrait and a mark, the hue is not spent on this — it stays as
   *  the ring around the picture, so the category colour survives the node
   *  carrying one. The image is sized to the shape's reach and not to the
   *  radius, or a diamond's points would be left bare by a picture fitted to
   *  the disc inside it. */
  _drawPhoto(n, r, shape) {
    if (r * this.camera.k < ART_MIN_PX) return;
    const rec = this.art.get(n.glyph);
    if (!rec || !rec.ready) return;
    const iw = rec.img.naturalWidth || 0;
    const ih = rec.img.naturalHeight || 0;
    if (!iw || !ih) return;
    ...
    const { ctx } = this;
    const inner = r - Math.max(1.5, r * 0.15);
    const span = inner * 2 * shapeReach(shape);

    ctx.save();
    tracePath(ctx, shape, n.x, n.y, inner);
    ctx.clip();
    const k = Math.max(span / iw, span / ih);
    ctx.drawImage(rec.img, n.x - (iw * k) / 2, n.y - (ih * k) / 2, iw * k, ih * k);
    ctx.restore();
  }
  ...
  _bindEvents() {
    ...
    const end = (ev) => {
      ...
      // Where it was dropped is where it stays.
